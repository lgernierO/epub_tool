...
"""
    ...
    def test_recognize_batch_decodes_each_sample(self):
        backend = OnnxGlyphOcrBackend.__new__(OnnxGlyphOcrBackend)
        backend.np = np
        backend.characters = ["blank", "你", "好", " "]
        backend.image_shape = [3, 48, 320]
        backend.image_mode = "RGB"
        backend.max_img_width = 3200
        backend.input_name = "x"

        class FakeSession:
            def run(self, _outputs, feeds):
                batch = feeds["x"]
                preds = np.zeros((batch.shape[0], 3, 4), dtype=np.float32)
                preds[0, 0, 1] = 0.9
                preds[1, 0, 2] = 0.95
                return [preds]
        ...
        images = [
            Image.new("RGB", (40, 80), (255, 255, 255)),
            Image.new("RGB", (50, 80), (255, 255, 255)),
        ]
        results = backend.recognize_batch(images)
        self.assertEqual([item.text for item in results], ["你", "好"])
